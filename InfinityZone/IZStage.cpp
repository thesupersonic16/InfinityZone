...
bool IZStage::LoadXML(string path)
{
    ...
    if (xml && size)
    {
        ...
        {
            IZStage_Unlock* unlock = FindUnlock(child->Name());
            if (unlock)
        ...
        for (auto child = xmlAssets->FirstChildElement(); child != NULL; child = child->NextSiblingElement())
